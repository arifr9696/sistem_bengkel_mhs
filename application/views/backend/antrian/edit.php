        ...
        <div class="col-md-9">
            <div class="card glass-card shadow-sm border-0">
                <div class="card-header bg-white border-bottom-0 pt-4 px-4">
                    <h4 class="page-title mb-1"><i class="bi bi-edit me-2"></i> Edit Antrean</h4>
                    <p class="text-muted small mb-0">Perbarui data antrean pelanggan.</p>
                </div>
                <div class="card-body px-4 pb-4">
                    <form action="<?= base_url('backend/antrian/edit/'.$antrian->id) ?>" method="POST">
                        <div class="mb-3">
                            <label>Pelanggan</label>
                            <select name="id_pelanggan" class="form-control" required>
                                <option value="">-- Pilih Pelanggan --</option>
                                <?php foreach ($pelanggan as $p): ?>
                                    <option value="<?= $p->id ?>" <?= ($p->id == $antrian->id_pelanggan) ? 'selected' : '' ?>>
                                        <?= $p->nama ?> (<?= $p->no_hp ?>)
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="id_kendaraan" class="form-control" required>
                                ...
                                <?php foreach ($kendaraan as $k): ?>
                                    <option value="<?= $k->id ?>" <?= ($k->id == $antrian->id_kendaraan) ? 'selected' : '' ?>>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="date" name="tgl_antrian" class="form-control" value="<?= $antrian->tgl_antrian ?>" required>
                        </div>

                        <div class="mb-3">
                            <label>Keluhan</label>
                            <textarea name="keluhan" class="form-control" rows="3" required><?= $antrian->keluhan ?></textarea>
                        </div>
